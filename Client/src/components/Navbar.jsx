...
export default function Navbar() {
     ...
     const linkStyle =
          "text-gray-700 hover:text-black dark:text-gray-300 dark:hover:text-white flex items-center gap-2 px-3 py-2 rounded-md transition-colors cursor-pointer";

     const activeLinkStyle =
          "bg-gray-500/20 dark:bg-[#00000091] dark:text-white";
     ...
     const navItems = [
          ...
          {
               label: 'Products',
               icon: <StorefrontIcon sx={{ fontSize: '1.2rem' }} />,
               path: '/products'
          },
          {
               label: 'About Us',
               icon: <Diversity3Icon sx={{ fontSize: '1.2rem' }} />,
               path: '/about'
          },
          {
               label: 'Contact Us',
               icon: <CallIcon sx={{ fontSize: '1.2rem' }} />,
               path: '/contact-us'
          },
     ];
     ...
     /*
      * IMPORTANT:
      * Navigate to the public home page FIRST.
      *
      * We intentionally delay handleUserLogout() by one event-loop tick.
      * This prevents ProtectedRoute/UserProfileLayout from seeing authUser=null
      * while we are still on a protected route and redirecting us to /login.
      */
     const handleLogout = () => {
          setOpen(false);
          setUserProfileDrawer(false);

          // Move to the public landing page first.
          navigate("/", { replace: true });

          // Open login dialog after logout navigation has started.
          setOpenLoginDialog(true);

          // Clear authentication after the route transition has been scheduled.
          setTimeout(() => {
               handleUserLogout();
     ...
     const handleRemoveNotification = async (index, mode) => {
          ...
          try {
               ...
               const res = await removeUserNotification(
                    authUser?._id,
                    mode,
                    index
               );
               ...
               if (res?.success) {
                    ...
                    if (mode === "all") {
                         ...
                         enqueueSnackbar(
                              res?.message || "Remove notification.",
                              { variant: "success" }
                         );
               ...
               enqueueSnackbar(
                    ...
                    "Server error while removing notification.",
                    { variant: "error" }
               );
     ...
     const displayLogo =
          theme === "light"
               ? company?.logoLightTheme
               : company?.logoDarkTheme;
          ...
          <>
               <nav className="sticky top-0 z-50 w-full shadow-md px-4 py-2 flex justify-between items-center transition-colors duration-300 bg-gray-100/70 text-black dark:bg-[#282828]/70 dark:text-white backdrop-blur-sm">
                    ...
                    <div className="hidden lg:flex gap-2 ml-4">
                         ...
                         {navItems.map((item, idx) => (
                              <Link
                                   key={item.path || idx}
                                   to={item.path}
                                   className={`${linkStyle} ${location.pathname.startsWith(item.path)
                                        ? activeLinkStyle
                                        : ''
                                        }`}
                              >
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                              ...
                              <Tooltip title="Cart">
                                   ...
                                   <button onClick={handleUserCart}>
                                        ...
                                        <div className="rounded-lg px-2 py-1 bg-[#FDE12D] flex items-center shadow-md">
                                             ...
                                             <span className="ps-1 font-semibold text-lg text-gray-700">
                                                  {cartItems?.length || 0}
                                             </span>
                         ...
                         <div className="relative inline-block">
                              ...
                              >
                                   ...
                                   {notification?.length > 0 &&
                                        authUser && (
                                             <span
                                                  className={`absolute top-0 right-0 font-bold px-1.5 h-fit bg-red-500 text-white rounded-md text-[12px] transition-all duration-300 ${animate
                                                       ? "animate-bounce"
                                                       : ""
                                                       }`}
                                             >
                         ...
                         <div className="bg-gray-500/10 hover:bg-gray-500/20 dark:bg-[#00000050] dark:hover:bg-[#00000090] rounded-full shadow">
                         ...
                         {notification.length === 0 ? (
                              ...
                              <ul className="space-y-3 px-3 py-3">
                                   ...
                                   {notification.map((item, idx) => (
                                        ...
                                        <li
                                             key={`${item?.title || "notification"}-${idx}`}
